...
Each fragrance also exposes its Pura-reported remaining percentage through a linked standard HomeKit Filter Maintenance service (for example, `Salt Remaining`). HomeKit uses “Filter Life” wording for this generic consumable-life service. The remaining level is associated with the Pura fragrance ID and follows the fragrance if the vial moves between bays.
...
- **Fragrance Controls (optional)**:
  ...
  - View the Pura-reported remaining percentage for each fragrance
